...
static int32_t lan8720_io_init(void);
static int32_t lan8720_io_deinit (void);

static int32_t lan8720_io_write_reg(uint32_t DevAddr, uint32_t RegAddr, uint32_t RegVal);
static int32_t lan8720_io_read_reg(uint32_t DevAddr, uint32_t RegAddr, uint32_t *pRegVal);

static int32_t lan8720_io_get_tick(void);

/* LAN8742 IO context object */
static lan8720_IOCtx_t  LAN8720_IOCtx = { lan8720_io_init,
                                          lan8720_io_deinit,
                                          lan8720_io_write_reg,
                                          lan8720_io_read_reg,
                                          lan8720_io_get_tick
                                        };
/* LAN8742 main object */
static lan8720_Object_t LAN8742;
...
int32_t nx_eth_phy_init(void)
{
    ...
    LAN8720_RegisterBusIO(&LAN8742, &LAN8720_IOCtx);
    /* Initialize the LAN8742 ETH PHY */

    if (LAN8720_Init(&LAN8742) == LAN8720_STATUS_OK)
    {
...
{
    return (LAN8720_SetLinkState(&LAN8742, LinkState));
}
...
{
    int32_t  linkstate = LAN8720_GetLinkState(&LAN8742);
...
/**
  ...
  * @retval 0 if OK, -1 if ERROR
  */
int32_t lan8720_io_deinit (void)
{
    return ETH_PHY_STATUS_OK;
}
...
/**
  ...
  * @retval 0 if OK -1 if Error
  */
int32_t lan8720_io_read_reg(uint32_t DevAddr, uint32_t RegAddr, uint32_t *pRegVal)
{
...
int32_t lan8720_io_write_reg(uint32_t DevAddr, uint32_t RegAddr, uint32_t RegVal)
{
...
/**
  ...
  * @retval Time value
  */
int32_t lan8720_io_get_tick(void)
{
  return HAL_GetTick();
}
