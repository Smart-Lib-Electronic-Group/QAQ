...
static UINT udp_tx_test_time   = 60;
static UINT udp_rx_test_time   = 60;
static UINT tcp_tx_test_time   = 60;
static UINT tcp_rx_test_time   = 60;
...
void nx_iperf_entry(NX_PACKET_POOL* pool_ptr, NX_IP* ip_ptr, UCHAR* http_stack, ULONG http_stack_size, UCHAR* iperf_stack, ULONG iperf_stack_size)
{
  ...
  status                   = nx_tcp_socket_create(nx_iperf_test_ip, &tcp_server_socket, "TCP Server Socket", NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, 32 * 1024, NX_NULL, nx_iperf_tcp_rx_disconnect_received);
  ...
  status = nx_tcp_socket_create(nx_iperf_test_ip, &tcp_client_socket, "TCP Client Socket", NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, 32 * 1024, NX_NULL, NX_NULL);
...
static void nx_iperf_check_token_value(char* token, char* value_ptr, ctrl_info* ctrl_info_ptr)
{
  ...
  /* Check for name. */
  if ((token_length == sizeof("TestType") - 1) && (memcmp(token, "TestType", token_length) == 0))
  {
    /* Check for value. */
    ctrl_info_ptr->ctrl_sign = UNKNOWN_TEST;
    if (value_ptr)
    {
      if ((value_length == sizeof(TCP_Rx) - 1) && (memcmp(value_ptr, TCP_Rx, value_length) == 0))
      {
        ctrl_info_ptr->ctrl_sign = TCP_Rx_Ready;
      }
      else if ((value_length == sizeof(TCP_Tx) - 1) && (memcmp(value_ptr, TCP_Tx, value_length) == 0))
      {
        ctrl_info_ptr->ctrl_sign = TCP_TX_START;
      }
      else if ((value_length == sizeof(UDP_Rx) - 1) && (memcmp(value_ptr, UDP_Rx, value_length) == 0))
      {
        ctrl_info_ptr->ctrl_sign = UDP_Rx_Ready;
      }
      else if ((value_length == sizeof(UDP_Tx) - 1) && (memcmp(value_ptr, UDP_Tx, value_length) == 0))
      {
        ctrl_info_ptr->ctrl_sign = UDP_TX_START;
      }
    }
  }
  else if ((token_length == (sizeof("ip") - 1)) && (memcmp(token, "ip", token_length) == 0))
  {
    char* ptr = value_ptr;
  ...
  else if ((token_length == (sizeof("test_time") - 1)) && (memcmp(token, "test_time", token_length) == 0))
  {
  ...
  else if ((token_length == (sizeof("rate") - 1)) && (memcmp(token, "rate", token_length) == 0))
  {
  ...
  else if ((token_length == (sizeof("size") - 1)) && (memcmp(token, "size", token_length) == 0))
  {
  ...
  else if ((token_length == (sizeof("port") - 1)) && (memcmp(token, "port", token_length) == 0))
  {
...
UINT nx_iperf_get_notify(NX_WEB_HTTP_SERVER* server_ptr, UINT request_type, CHAR* resource, NX_PACKET* packet_ptr)
{
  ...
  if (((length == sizeof("/test.htm") - 1) && (memcmp(resource, "/test.htm", length) == 0)) || ((length == 1) && *resource == '/'))
  {
    /* Printer the header.  */
...
static void nx_iperf_send_image(NX_WEB_HTTP_SERVER* server_ptr, UCHAR* img, UINT imgsize)
{
  ...
  status = nx_web_http_server_callback_generate_response_header(server_ptr, &resp_packet_ptr, NX_WEB_HTTP_STATUS_OK, imgsize, "image/jpeg", NX_NULL);
  if (status)
  {
    return;
  }
    ...
    case TCP_Rx_Ready :
      {
        ...
        {
          iperf_ctrlInfo_ptr->TestTime = 60 * NX_IP_PERIODIC_RATE;
        }
...
void nx_iperf_tcp_rx_test(UCHAR* stack_space, ULONG stack_size)
{
  ...
  status = tx_thread_create(&thread_tcp_rx_iperf, "thread tcp rx", nx_iperf_thread_tcp_rx_entry, (ULONG)&nx_iperf_ctrl_info, stack_space, stack_size, NX_IPERF_THREAD_PRIORITY, NX_IPERF_THREAD_PRIORITY, TX_NO_TIME_SLICE, TX_AUTO_START);
...
void nx_iperf_tcp_tx_test(UCHAR* stack_space, ULONG stack_size)
{
  ...
  status = tx_thread_create(&thread_tcp_tx_iperf, "thread tcp tx", nx_iperf_thread_tcp_tx_entry, (ULONG)&nx_iperf_ctrl_info, stack_space, stack_size, NX_WEB_HTTP_SERVER_PRIORITY + 1, NX_WEB_HTTP_SERVER_PRIORITY + 1, TX_NO_TIME_SLICE, TX_AUTO_START);
...
void nx_iperf_udp_rx_test(UCHAR* stack_space, ULONG stack_size)
{
  ...
  status = tx_thread_create(&thread_udp_rx_iperf, "thread udp rx", nx_iperf_thread_udp_rx_entry, (ULONG)&nx_iperf_ctrl_info, stack_space, stack_size, NX_WEB_HTTP_SERVER_PRIORITY + 1, NX_WEB_HTTP_SERVER_PRIORITY + 1, TX_NO_TIME_SLICE, TX_AUTO_START);
...
void nx_iperf_udp_tx_test(UCHAR* stack_space, ULONG stack_size)
{
  ...
  status = tx_thread_create(&thread_udp_tx_iperf, "thread udp tx", nx_iperf_thread_udp_tx_entry, (ULONG)&nx_iperf_ctrl_info, stack_space, stack_size, NX_WEB_HTTP_SERVER_PRIORITY + 1, NX_WEB_HTTP_SERVER_PRIORITY + 1, TX_NO_TIME_SLICE, TX_AUTO_START);
